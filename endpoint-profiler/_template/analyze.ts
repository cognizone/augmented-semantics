 ...
 *
 * Run with: npx tsx analysis/_template/analyze.ts
 */
...
const enrichReport = async (report: TypeReport): Promise<TypeReport> => {
  ...
  const properties = await Promise.all(report.properties.map(async (prop) => {
    ...
    let enrichedDatatypes: Array<{ datatype: string; datatypeQName?: string; count: number; languages?: Array<{ lang: string; count: number }> }> | undefined
    if (prop.datatypes) {
      enrichedDatatypes = await Promise.all(prop.datatypes.map(async (dt) => {
        if (dt.datatype === 'iri') {
          return { datatype: dt.datatype, count: dt.count }
        }
