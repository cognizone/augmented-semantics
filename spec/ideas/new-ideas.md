New ideas: copy them into ideas.md, remove the list here (but keep the file and the instructions) then work on them.

- notation of concepts in "Classification des accords sectoriels avec l'Union européenne" just show "id" as type?
- packaging needs to be reviewed. system complains
- still recent concepts does not use full vertical space!!!
- we should add the type somewhere in the "main component part" (typically concept?!)
    as au side issue one we are there (in inconsistent state) we cannot select other concept scheme anymore
- using endswith logic to find the properties??
- navigation to related concepts in same database should be via the tool. for example the main taxonomy + it/fr/de taxosare
- search config icon is too big
- add an option to explicitely show xsd:string 
- deprecated label is hard to see in light mode
- build reusable component for "clear history" and "delete endpoint" popup? does it make sense?
- user manual images
- does not jump to the correct location in the tree
- problem with long list of languages (agrovoc only has 10?)
- switching endpoint should clear both tree as right part
- switching language works more or less ok but tree is not correct
- improve orphan sparql cfr better-sparql-for-orphans.sparql
- when no endpoints give like a welcome splash page with the list of curated endpoints to add!
- autocomplete in scheme dropdown?
- if no endpoints show splashscreen to add endpoints?